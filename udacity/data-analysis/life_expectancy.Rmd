...
ggsave("GDP Multiple Countries in 1 plot.png")
```
```{r}
df2 <- data.frame(Year = gdp$Year, Country = c(gdp$Germany, gdp$`United States`, gdp$China, gdp$Russia))
df.melted <- melt(df,id="Year")

ggplot(df, aes(x=factor(Year), y = c(Germany, USA, Russia, China))) + 
